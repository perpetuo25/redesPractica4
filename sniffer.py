...
.-._} } |\  || || |  | |  | {__ | .-. \
`----'`-' `-'`-'`-'  `-'  `----'`-' `-'
Comentado por Sanchez Sarmiento Eric

"""
#Definimos tabuladores para usarlos mas comodamente
TAB_1 = '\t - '
TAB_2 = '\t\t - '
TAB_3 = '\t\t\t - '
TAB_4 = '\t\t\t\t - '
...
"""Imprime los paquetes en ipv4 dependiendo del protocolo"""
...
"""Imprime los paquetes en ipv6 dependiendo del protocolo"""
...
"""Imprime las cabeceras de icmpv6"""
...
"""Devuelve el siguinte header dependido del que se haya recibido"""
...
"""Formato para la dirección mac"""
def get_mac_addr(bytes_addr):
    bytes_str = map('{:02x}'.format, bytes_addr)
    mac_addr = ':'.join(bytes_str).upper()
    return mac_addr

"Desempaqueta los paquetes recibidos en ipv6"""
...
"""Regresa la direccion ip en un formato amigable"""
def ipv4(addr):
    return '.'.join(map(str, addr))

"""Desempaqueta todos los paqutes de ICMP"""
def icmp_packet(data):
    icmp_type, code, checksum = struct.unpack('! B B H', data[:4])
    return icmp_type, code, checksum, data[4:]

"""Desempaqueta todos los paqutes de TCP"""
def tcp_seg(data):
...
"""Desempaqueta todos los paqutes de UDP"""
def udp_seg(data):
